...
export function StaticSplitWorld({
  ...
  return (
    <section
      className="relative overflow-hidden border border-onyx-700 bg-onyx-800"
    ...
    >
      <div className="grid lg:grid-cols-[1fr_auto_1fr]">
        <div className="bg-his-500/45 p-5 md:p-8">
          ...
          <h3 className="mt-6 font-display text-4xl leading-none text-parchment">
            {his?.name}
          </h3>
        ...
        <div className="flex min-h-32 flex-col items-center justify-center border-y border-gold-300/35 bg-onyx-900 p-5 text-center lg:border-x lg:border-y-0">
          <p id="split-world-title" className="type-eyebrow text-gold-300">
            Shared accord
          </p>
          <p className="mt-3 font-display text-3xl leading-none text-parchment">
            {sharedNotes.map((note) => note.name).join(" + ")}
          </p>
          <ButtonLink href={`/pairs/${pair.slug}`} className="mt-6">
            Explore pair
          </ButtonLink>
        </div>
