...
import { buildWhatsAppOrderMessage, buildWhatsAppOrderUrl } from "@/lib/whatsapp/whatsapp-order";
...
describe("WhatsApp Order URL & Message Generation", () => {
  ...
  it("generates a structured message with all required brand details", () => {
    ...
    expect(message).toContain("https://thescentpair.com/fragrance/midnight-oath");
  });

  it("builds a valid wa.me URL with clean phone number and encoded payload", () => {
    const { url, refCode } = buildWhatsAppOrderUrl(sampleDetails, "+234 801 234 5678");
